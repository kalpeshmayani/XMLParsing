...
public class ItemList extends Activity implements OnClickListener, OnRefreshListener, XmlWSAsync {
	...
	@Override
	public void onXmlWSResponse(String serviceType, ArrayList<XmlItem> xmlItems, Exception error) {
		
		if (error == null) {

			if (serviceType.equalsIgnoreCase(WSConstant.XML_RT_GET_ITEM_LIST)) {
				showData(xmlItems);
			}
	...
	private void showData(ArrayList<XmlItem> xmlItems) {
		
		list = xmlItems;
		
		if(list == null) {
			list = new ArrayList<XmlItem>();
		}
